...
def update_appt_status(appt_id, new_status):
    valid_statuses = ['Scheduled', 'Sent to Counsellor', 'In Session', 'Completed', 'Cancelled']
    if new_status not in valid_statuses:
        # Check underscore version for backward compat if needed or just handle it
        mapping = {
            'scheduled': 'Scheduled',
        ...
        if new_status in mapping:
            ...
            flash("Invalid workflow state.", "error")
            return redirect(url_for('dashboard'))
        
    try:
        conn = get_db_connection()
        # Security: Log the transition
        conn.execute(
        ...
        conn.execute(
            ...
            (session.get('user_id'), 'WORKFLOW_TRANSITION', f"Appt {appt_id} moved to {new_status}")
        )
        conn.commit()
        conn.close()
        flash(f"Status updated to: {new_status.replace('_', ' ').title()}", "success")
    except Exception as e:
        flash(f"Workflow error: {str(e)}", "error")
